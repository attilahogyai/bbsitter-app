...
export default FormController.extend({
  ...
  user:null,
  addLanguage:null,
  requiredFields:['name','email','birthYear','sex','country','targetLocations','educationLevel','education','languages','intProp1','experience','unitPrice','currency','driveLicense','ownCar','description'],
  ...
  }.property('instance.name','user.email'),
  email:function(key, value, previousValue){
    if (arguments.length > 1) {
  ...
  }.property('instance.email','user.email'),
  enableLocationInput:function(){
    if(this.get('instance.serviceLocationType.id')==="1" || this.get('instance.serviceLocationType.id')==="4"){
      return true;
    }
    return false;
  }.property('instance.serviceLocationType'),
  ...
  actions:{
    ...
    showProfessions:function(){
      var alertWindow=Em.$('#profession-popup');
