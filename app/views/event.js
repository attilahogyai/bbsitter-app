...
var EventView=Em.View.extend({
	...
	setPosition: function(){
		...
		var offsetStart = Em.$('#'+startId).position();
		var offsetEnd = Em.$('#'+endId).position();
		if(offsetStart && offsetEnd){
			var height = offsetEnd.top - offsetStart.top;
			var width = Em.$('#'+startId).width();
	...
	didInsertElement: function(){
		...
		Ember.run.scheduleOnce('afterRender', function(){
			c._super();
			c.setPosition();
			Em.addChangeSizeListener(function(){
	...
	willClearRender: function(){
		this._super();
		Em.removeChangeSizeListeners(this);
		this.setPosition();
	}
});
